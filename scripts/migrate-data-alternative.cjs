...
// Función para cargar datos compilados
async function loadCompiledData() {
  try {
    // Intentar cargar desde dist o directamente desde src
    let allMeals, allSupplements, allTips;

    try {
      const recipesModule = await import("../src/data/recipes.js");
      const supplementsModule = await import("../src/data/supplements.js");
      const tipsModule = await import("../src/data/tips.js");

      allMeals = recipesModule.allMeals;
      allSupplements = supplementsModule.allSupplements;
      allTips = tipsModule.allTips;
    } catch (e) {
      // Fallback: importar datos directamente (si están compilados)
      console.log("⚠️  Usando datos de muestra para testing...");
      ...
      allSupplements = [
        {
          ...
          description: "Suplemento de testing",
          categoria: "Proteína",
        },
      ];
      ...
      allTips = [
        {
          id: "test-tip",
          title: "Tip de prueba",
          content: "Este es un tip de prueba",
          tags: ["General"],
        },
      ];
    }

    return { allMeals, allSupplements, allTips };
  } catch (error) {
...
async function runMigration() {
  try {
    ...
    // Cargar datos
    const { allMeals, allSupplements, allTips } = await loadCompiledData();
